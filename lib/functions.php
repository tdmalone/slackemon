...
function slackemon_debug_backtrace( $levels = 3, $skip_first = false, $ignore_args = true ) {
  ...
  // Do a backtrace with the requested number of levels plus one (unless 0 was used for unlimited levels).
  // We then drop the trace level that was generated by this current function.
  $backtrace = debug_backtrace( $ignore_args ? DEBUG_BACKTRACE_IGNORE_ARGS : null, $levels ? $levels + 1 : $levels );
